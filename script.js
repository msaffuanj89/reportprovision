...
async function downloadPDF(){
  ...
  const originalPages = Array.from(document.querySelectorAll("#pdfArea .page"));
  ...
  // Build a clean fixed-size PDF stage so responsive screen scaling will not affect PDF.
  ...
  const clonedPages = originalPages.map((page) => {
    ...
    // Remove UI-only controls from PDF.
    clone.querySelectorAll(".photoTools, .phoneTabs, button, input, textarea").forEach(el => el.remove());

    // Remove empty photo cards.
    ...
    clone.querySelectorAll(".photo span").forEach(el => el.remove());

    // If no caption, remove caption box.
    clone.querySelectorAll(".captionBox").forEach(el => {
      if(!el.textContent.trim()) el.remove();
    });

    // If no description, remove blank description.
    clone.querySelectorAll(".photoDesc").forEach(el => {
      if(!el.textContent.trim()) el.remove();
    });

    // Recalculate photo grid based on remaining cards.
  ...
  try{
    ...
    if(!jsPDF || typeof html2canvas === "undefined"){
      throw new Error("PDF library belum loaded. Pastikan internet aktif kerana library PDF guna CDN.");
    }
    ...
    for(let i = 0; i < clonedPages.length; i++){
      const canvas = await html2canvas(clonedPages[i], {
        scale: 2,
        useCORS: true,
        allowTaint: true,
